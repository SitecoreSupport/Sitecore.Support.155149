...
namespace Sitecore.Support.SessionProvider.MongoDB
{
  ...
  internal sealed class SimpleMongoSessionStateStore : IMongoSessionStateStore
  {
    ...
    public SessionStateStoreData GetItem(string application, string id, out SessionStateLockCookie lockCookie, out int flags)
    {
    ...
    public SessionStateStoreData GetItemExclusive(string application, string id, SessionStateLockCookie lockCookie, out int flags)
    {
      Assert.ArgumentNotNull(application, "application");
      Assert.ArgumentNotNull(id, "id");
    ...
    /// </remarks>
    public SessionStateStoreData GetExpiredItemExclusive(string application, DateTime signalTime, SessionStateLockCookie lockCookie, out string id)
    {
      Assert.ArgumentNotNull(application, "application");
      Assert.ArgumentNotNull(lockCookie, "lockCookie");
    ...
    public SessionStateLockCookie GetItemLock(string application, string id)
    {
      Assert.ArgumentNotNull(application, "application");
      Assert.ArgumentNotNull(id, "id");

      SessionStateLockCookie result = this.m_Store.GetItemLock(application, id);

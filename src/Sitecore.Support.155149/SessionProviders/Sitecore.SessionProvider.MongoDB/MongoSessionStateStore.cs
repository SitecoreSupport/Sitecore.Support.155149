...
namespace Sitecore.Support.SessionProvider.MongoDB
{
  ...
  internal class MongoSessionStateStore
  {
    ...
    internal SessionStateStoreData GetItem(string application, string id, out SessionStateLockCookie lockCookie, out int flags)
    {
      ...
      if (famr.Ok && (null != famr.ModifiedDocument))
      {
        ...
        {
          lockCookie = new SessionStateLockCookie(lockId, lockTs);
        }

        result = SessionStateSerializer.Deserialize(famr.ModifiedDocument[FIELD_DATA].AsByteArray);
    ...
    /// </returns>
    internal SessionStateStoreData GetItemExclusive(string application, string id, SessionStateLockCookie lockCookie, out int flags)
    {
      Assert.ArgumentNotNull(application, "application");
      Assert.ArgumentNotNull(id, "id");
    ...
    internal SessionStateLockCookie GetItemLock(string application, string id)
    {
      Assert.ArgumentNotNull(application, "application");
      Assert.ArgumentNotNull(id, "id");

      SessionStateLockCookie result = null;
      ...
      if (null != document)
      {
        ...
        result = new SessionStateLockCookie(lockCookie, lockTimestamp);
      }
    ...
    [CanBeNull]
    internal SessionStateStoreData GetExpiredItemExclusive([NotNull] string application, [NotNull] SessionEndCandidate candidate, [NotNull] SessionStateLockCookie lockCookie)
    {
